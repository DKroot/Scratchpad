...
*/
DECLARE cursor_tmp_demo_cursors CURSOR LOCAL STATIC FORWARD_ONLY READ_ONLY FOR --
  SELECT tour_id, group_id, year, city
  FROM #demo_cursors
DECLARE @tour_id INT, @group_id INT, @year INT, @city NVARCHAR(255)
OPEN cursor_tmp_demo_cursors
PRINT '## Resultset ##'
WHILE 1 = 1 BEGIN
  FETCH NEXT FROM cursor_tmp_demo_cursors INTO @tour_id, @group_id, @year, @city
  IF @@fetch_status <> 0
    BREAK
  PRINT concat_ws(', ', @tour_id, @group_id, @year, @city)
END
CLOSE cursor_tmp_demo_cursors