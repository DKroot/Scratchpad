-- Deltas
SELECT month, sales,
  lag(sales, 1, 0) OVER (ORDER BY month) AS prev,
  lead(sales, 1, 0) OVER (ORDER BY month) AS next,
  sales - lag(sales, 1, 0) OVER (ORDER BY month) AS delta_with_prev,
  sales - lead(sales, 1, 0) OVER (ORDER BY month) AS delta_with_next
  FROM
    (
      VALUES
        (201901, 70000),
...
SELECT
  employees.employee AS employee, employees.employee_id, (
  SELECT sales.month
    FROM
      (
        VALUES
...
SELECT employee, employee_id, month AS largest_sales_month
  FROM
    (
      SELECT
        employees.employee AS employee, employees.employee_id, sales.month,
