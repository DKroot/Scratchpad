CREATE OR REPLACE PACKAGE BODY ned_data_mask_pkg AS
  ...
  IS
    ...
    includedPattern ned_masked_data_formats.masking_pattern%TYPE;
  BEGIN
    LOOP
...
--       debug(utl_lms.format_message('#..# a value from the %s dictionary', domain));
  ...
  BEGIN
    ...
    FOR maskedColumn IN (
      ...
      FOR rowSet IN (
        SELECT nmd.row_set_num, nmd.row_filter_sql, nmd.null_percentage, nmdf.masking_pattern
        ...
        IF rowSet.masking_pattern IS NULL AND rowSet.null_percentage < AllNullsPercentage THEN
          raise_application_error(CustomErrorCode, utl_lms.format_message(
        ...
        debug(utl_lms.format_message('Row set #%s: %s%s%s', to_char(rowSet.row_set_num),
                                     ...
                                     CASE
                                       WHEN rowSet.masking_pattern IS NULL THEN ''
                                       ELSE ', masking pattern = `' || rowSet.masking_pattern || '`'
                                     END));
        effectiveNullPercentage := CASE maskedColumn.nullable WHEN 'Y' THEN rowSet.null_percentage ELSE 0 END;
        effectivePattern := CASE
                              WHEN effectiveNullPercentage < AllNullsPercentage THEN effective_pattern(rowSet.masking_pattern)
                              ELSE NULL
                            END;
