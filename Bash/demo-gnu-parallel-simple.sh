...
echo -e "\n### Parallelize a sequence ###"
# shellcheck disable=SC2016
parallel --halt soon,fail=1 --line-buffer --tagstring '|job#{#}/{= $_=total_jobs() =} s#{%}|' "set -e
    echo 'Processing {} ...'
    echo '{}: done.'" ::: {1..10}
#-verbose
...
parallel --halt soon,fail=1 --line-buffer --tagstring '|job#{#} s#{%}|' \
...
# GNU parallel will stop spawning new jobs with --halt soon,fail=1
# With --halt now,fail=1 the running jobs will be killed immediately
parallel --halt soon,fail=1 --line-buffer --tagstring '|job#{#} s#{%}|' echo "Processing '{}' ..." ::: \
  arg1 'arg(2)' arg3

echo -e "\n### Simple command piping arguments to stdout ###"
parallel --halt soon,fail=1 --line-buffer --tagstring '|job#{#} s#{%}|' 'echo {} | cat' ::: \
  arg1 'arg(2)' arg3

echo -e "\n### A command with multi-line arguments ###"
parallel --null --halt soon,fail=1 --line-buffer --tagstring '|job#{#} s#{%}|' \
  echo "Processing {} ..." ::: '1: line1
1: line2' '2: line 1
2: line 2'

echo -e "\n### Processing multiple arguments at a time ###"
parallel --halt soon,fail=1 --line-buffer --tagstring '|job#{#} s#{%}|' "set -e;
